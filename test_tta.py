"""
Evaluate a trained SAM3 PEFT model with test-time augmentation (TTA).

Same interface as test.py but runs 8 augmented views per image (D4: identity,
rotations 90°/180°/270°, horizontal flip and their combinations) and merges
predictions by mean of softmax logits. Use for stronger evaluation metrics.

Usage:
    python test_tta.py --config configs/sam3_lora_potsdam.yaml --checkpoint experiments/.../best.pth
"""
...
from utils.tta import run_tta, tta_d4_transform

# Dataset class colors (ignore + 6 ISPRS classes)
CLASS_PALETTE = [
    (64, 64, 64),    # 0 ignore
...
def mask_to_rgb(mask: np.ndarray, palette: list, num_class_indices: int) -> np.ndarray:
    """Convert class index mask [H,W] to RGB [H,W,3] using palette."""
    h, w = mask.shape
    rgb = np.zeros((h, w, 3), dtype=np.uint8)
    for c in range(min(num_class_indices, len(palette))):
        rgb[mask == c] = palette[c]
    if mask.size > 0 and mask.max() >= len(palette):
        for c in range(len(palette), int(mask.max()) + 1):
            rgb[mask == c] = (128, 128, 128)
    return rgb


parser = argparse.ArgumentParser(description="SAM3 PEFT evaluation with TTA")
...
eval_dir = setup_run_log(
    exp_dir_base,
    f"eval_tta_{timestamp()}",
    log_filename="eval_tta.log",
    use_logging=False,
)
print(f"Eval run directory: {eval_dir}")
print("TTA: D4 (8 views)")
...
tta_transforms = tta_d4_transform()
...
with torch.no_grad():
    for batch in tqdm(loader, desc="Evaluating (TTA)"):
...
# Visualize 10 random samples (input | ground truth | prediction) with TTA
...
print("\n" + "=" * 60)
print(f"Results (TTA D4) — {ds_cfg.type} ({cli.split}), method={method}")
print("=" * 60)
