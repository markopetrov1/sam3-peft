# Massachusetts Buildings Dataset — 1500x1500 -> 512x512 tiles (MMSeg format).
# Split is taken from metadata.csv (source of truth). Same tiling as Roads:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Convert Massachusetts Buildings Dataset to MMSeg format (512x512 tiles) using metadata.csv"
    )
    parser.add_argument(
        "dataset_path",
        help="Path to Massachusetts-Buildings-Dataset root (contains metadata.csv, tiff/)",
    )
    parser.add_argument(
        "-o", "--out_dir",
        default=None,
        help="Output root (default: dataset_path/../massachusetts_buildings_mmseg)",
    )
    parser.add_argument(
        "--clip_size",
        type=int,
...
def main():
    ...
    if args.out_dir is None:
        out_dir = osp.join(osp.dirname(dataset_path), "massachusetts_buildings_mmseg")
    else:
